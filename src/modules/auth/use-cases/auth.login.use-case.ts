...
import { type IGetAuthUserUseCase, GET_AUTH_USER_USE_CASE } from "src/modules/user/interfaces/use-cases/iget.auth.user.use-case";
import { type ITokenGenerator, TOKEN_GENERATOR } from "../interfaces/auth/itoken.generator";
...
export class AuthLoginUseCase implements IAuthLoginUseCase{
    ...
    async execute(request: AuthUserRequestDTO): Promise<LoginDTO | never> {
        const user: AuthUserDTO | never = await this.getAuthUserUseCase.execute(request)
